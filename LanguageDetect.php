...
class LanguageDetect
{
    ...
    {
        $language = self::fontDetect($lang);
        if ($language == 'zawgyi') {
            $changedlang = self::zg2uni($lang);
            return $changedlang;
        }else{
            return $lang;
        }
    ...
    {
        if (strpos($pattern, self::WHITESPACE) !== false) {
            $pattern = str_replace(self::WHITESPACE, '', $pattern);
            $pattern = json_decode('"' . $pattern . '"');
            $pattern = "~" . self::WHITESPACE . $pattern . "~u";
        } else {
    ...
    public static function fontDetect(string $content, $default = 'zawgyi')
    {
        ...
        $unicode = $zawgyi = 0;
        foreach (self::$ruleSet as $fontRule => $ruleSet) {
            $$fontRule = 0;
            foreach ($ruleSet as $key => $rule) {
                if (strpos($rule, chr(13)) !== false) {
                    $rule = self::parseline($rule);
                }
